...
func FrenzelPompeMultivariate(xyz [][]float64, xIndices, yIndices, zIndices []int64, k int64) []float64 {

	r := make([]float64, len(xyz), len(xyz))

	h_k := continuous.Harmonic(k-1) / float64(len(xyz))
